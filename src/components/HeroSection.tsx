        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <div className="inline-flex items-center gap-2 bg-card border border-border rounded-full px-4 py-1.5 mb-6">
              ...
              <span className="text-xs font-body font-medium text-muted-foreground">Trusted by 500+ Schools Across India</span>
            </div>

            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-display font-bold text-foreground leading-tight mb-6">
              School Accounting,{" "}
              <span className="text-gradient-gold">Simplified</span> with Tally Prime
            </h1>

            <p className="text-lg text-muted-foreground font-body leading-relaxed mb-8 max-w-lg">
              The only school management software that seamlessly integrates with Tally Prime — automate fees, payroll, expenses, and compliance in one powerful platform.
            </p>

            <div className="flex flex-col sm:flex-row gap-4">
              <Button size="lg" className="gradient-gold text-accent-foreground font-body font-semibold shadow-gold hover:opacity-90 transition-opacity text-base px-8" onClick={onBookDemo}>
                Book a Free Demo
                <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
              <Button variant="outline" size="lg" className="font-body text-base">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.2 }} className="relative">
            <div className="relative rounded-2xl overflow-hidden shadow-premium">
              <img src={heroImage} alt="SchoolTally - School Accounting Software integrated with Tally Prime" className="w-full h-auto" />
            </div>
